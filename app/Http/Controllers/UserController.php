...
class UserController extends Controller
{
    ...
    // Take data from registration form and create account/send email
    ...
    public function postAccount(Request $request)
    {
        ...
        if (Hash::check($old_password, $user->getAuthPassword())) {
            ...
            return redirect()->back()->with('global', 'Старый пароль введен не верно');
        }
    }
    ...
    public function postForgot(Request $request)
    {
        ...
        if ($user->count()) {
            ...
            if ($user->save()) {
                Mail::send('emails.auth.recover', ['link' => URL::route('recover', $code), 'password' => $password,
                    'username' => $user['username']], function ($message) use ($data, $user) {
